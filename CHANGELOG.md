...
- Review hardening: `Game.reset()` starts fresh id-stable episodes on the same node process (the fast path for training loops); `WsBridge` survives tab refreshes cleanly (stale connections closed on re-accept, socket errors surface as `BridgeError` with the connection marked dead instead of leaking raw `OSError`s); `new_game` validates its `control` list before touching game state; optional `python/pyproject.toml` for `pip install -e python/`.
