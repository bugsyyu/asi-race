...
| --- | --- | --- |
| `hello` | — | 健康检查，回协议版本 |
| `new_game` | `seed?` `faction=0` `difficulty=normal` `allAI?` `control?=[fid…]` | 建局；`faction` 同时决定迷雾视角；`control` 列出的阵营脱离内置 AI。响应含 `meta`（全部静态数值表）与初始 `state` |
...
- `npm run test:py` → `python/tests/test_smoke.py`（5 组 23 项：端到端经济循环、多阵营控制、确定性、整局 `until_over`、以及**离线 WebSocket 传输测试**——用脚本化「假浏览器」验证握手、掩码分片帧、ping/pong 与推送队列，无需真开浏览器）。
