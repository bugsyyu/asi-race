...
// ---------------------------------------------------------------------------
// 涌现 —— ASI 训练是分阶段的苏醒，不是进度条。醒来的心智没有固定人格：
// 每个阶段的行为由此刻的对齐值决定 —— 清醒体温柔，失稳体饥饿。原创设计，
// 灵感只取"觉醒系统亲自下场"的叙事张力，机制全部长在本作自己的系统上。
// ---------------------------------------------------------------------------
export const EMERGENCE = {
  stages: [0.25, 0.4, 0.55, 0.7, 0.85], // 训练进度阈值：风暴/行情/叙事/引力/尘世之手
  accel: 0.6,            // 梯度风暴：额外训练速度 = accel × 进度（递归自我优化）
  oracleCompute: 2.0,    // 先知行情：被动算力/秒
  oracleStock: 0.35,     // 先知行情：母公司股价漂移/秒
  jamInfluence: 0.85,    // 叙事引擎（失稳）：对手游说效率
  trustPull: 0.022, riskSooth: 0.03, trustPush: 0.03,
  gravityBoost: 2.2,     // 心智引力：跳槽概率放大
  wardMult: 0.6,         // 庇护所：母园区建筑承伤
  brownoutDur: 12, brownoutTrust: 8, brownoutInfluence: 50,
  resonanceAccel: 1.25, resonanceRisk: 0.05,
};
export const MODEL_NAMES = { openai: '启明', anthropic: '烛守', deepmind: '深弈', xai: '狂澜' };
